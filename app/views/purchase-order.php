          ...
          <table class="table table-striped jambo_table bulk_action">
            ...
            <tbody>
              <?php foreach($datagrid as $key => $row) :
              	$detail = ORM::for_table('purchase_transaction_detail')->raw_query("SELECT sum(QTY * PRICE - DISCOUNT) as TOTALPO FROM purchase_transaction_detail WHERE PUID = :puid", ['puid' => $row['UID']])->find_one();
              	$totalPurchase = 0;

              	if($detail->TOTALPO != null) $totalPurchase = $detail->TOTALPO;
              ?>
              <tr>
                  <td class="a-center ">
