<?php
	class cashSalesLogic extends baseLogic {
		public function run() {
			// setup ORM
			...
			// set title
			self::assign('title', 'Cash Sales');
			self::assign('dataProduct', []);
			
			// datagrid
			$datagrid = ORM::for_table('sales_transaction')->find_many();
		...
		public static function initial() {
			if(self::post('update')) {
				...
				$record = ORM::for_table('sales_transaction')->find_one($uid);
				...
				$recordDetail = ORM::for_table('sales_transaction_detail')->join('product', array('product.UID', '=', 'sales_transaction_detail.PROD'))->select($fields)->where("PUID", $uid)->find_many();
		...
		public static function register($dataProduct = []) {
			...
			// update or insert
			$record = ORM::for_table('sales_transaction')->create();
			...
			if(self::getBeans('UID') != null) {
				...
				ORM::for_table('sales_transaction_detail')->where('PUID', $uid)->delete_many();
			}
			...
			if(!self::hasMessage()) {
				...
				foreach($dataProduct as $key => $value) {
					$recordDetail = ORM::for_table('sales_transaction_detail')->create();
				...
				self::message("Register Cash Sales information done..");
				self::presistance();
				RedirectHelper::back();
			}else{
				self::setBeans('TYPEBEANS', 'RECOVERY');
		...
		public static function delete($uid) {
			ORM::for_table('sales_transaction')->find_one($uid)->delete();
			ORM::for_table('sales_transaction_detail')->where('PUID', $uid)->delete_many();
		...
		public static function delete_bulk($uidArray) {
			ORM::for_table('sales_transaction')->where_in('UID', $uidArray)->delete_many();
			ORM::for_table('sales_transaction_detail')->where_in('PUID', $uidArray)->delete_many();
