<?php
	class categoryLogic extends baseLogic {
		public function run() {
			...
			// set title
			self::assign('title', 'Category');

			// set inline editing
			if(self::post('inline_edit')) {
				self::featureInlineEdit('Category', 'UID');
			}

			// datagrid
			$datagrid = ORM::for_table('category')->find_many();
		...
		public static function initial() {
			if(self::post('update')) {
				...
				$record = ORM::for_table('category')->find_one($uid);

				// set update value
				self::setBeans('UID', $record->UID);
				self::setBeans('NMCATGRY', $record->NMCATGRY);
		...
		public static function register() {
			...
			// update or insert
			$record = ORM::for_table('category')->create();
			...
			// set record value
			$record->NMCATGRY = self::getBeans('NMCATGRY');
			...
			if(!self::hasMessage()) {
				...
				self::message("Register Category information done..");
		...
		public static function delete($uid) {
			ORM::for_table('category')->find_one($uid)->delete();
		...
		public static function delete_bulk($uidArray) {
			ORM::for_table('category')->where_in('UID', $uidArray)->delete_many();
		...
		public static function validation() {
			// validation
			if(self::getBeans('NMCATGRY') == "") {
				self::message("Error Name category blank !", "danger");
			}
